...
export type ShieldPhase = 'idle' | 'shield' | 'interval' | 'blocked'
...
export function phaseDuration(phase: ShieldPhase, p: ShieldParams): number {
  if (phase === 'shield' || phase === 'blocked') return p.shieldDuration
...
export function resistRemaining(state: ShieldState, now: number): number {
  return remainingSince(state.lastDispelAt, DISPEL_RESIST, now)
}

// 玩家技能冷卻剩餘
export function cooldownRemaining(state: ShieldState, now: number): number {
...
export function startBlocked(now: number): ShieldState {
  ...
  while (s.phase !== 'idle' && now >= phaseEnd(s, p)) {
    ...
    if (s.phase === 'shield') {
      ...
      s = s.dispelValid
        ...
        : { phase: 'shield', phaseStart: end, dispelValid: false, lastDispelAt: s.lastDispelAt }
    } else {
      // blocked → 下一個間隔
      s = { phase: 'interval', phaseStart: end, dispelValid: false, lastDispelAt: s.lastDispelAt }
    }
  ...
  while (out.length < count && s.phase !== 'idle') {
    ...
    if (s.phase === 'shield') {
      ...
      if (s.dispelValid) {
        ...
        s = { phase: 'shield', phaseStart: end, dispelValid: false, lastDispelAt: s.lastDispelAt }
      }
    } else {
