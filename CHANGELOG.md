...
- 5dcea14: 紀錄可標記「擱置」（暫不列入未領總攬，列表顯示擱置徽章）；未領總攬每場新增「開啟 ↗」可開新分頁直達該紀錄的分配名單；紀錄狀態跨分頁即時同步

## 1.7.0

### Minor Changes

- a58bfa5: 新增「未領總攬」頁：未結清款項依團員分區塊、逐行複製、結清標記、待售標註；品名 autocomplete 改模糊比對（前綴/子字串/子序列計分）；品名標準化一次性遷移（潛能**%、手攻**%、大師附加、附加奇幻）
